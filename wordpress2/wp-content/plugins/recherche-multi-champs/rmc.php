	...
	if ((($post_type == "post") && ($afficher_champs_articles == "on")) || (($post_type == "page") && ($afficher_champs_pages == "on"))){
		...
		foreach ($resultats as $cv){
			...
			if ( isset($custom_fields[$cv->cle][0]) ) {
				...
				if (($options['afficher_champs_vide'] == true) || ($custom_fields[$cv->cle][0] != ""))
...
function rmc_search_shortcode($atts){
	...
	$echo = '<form id="rmc_form_id" class="rmc_form" action="" method="post" style="width:'.$size.'"><input type="hidden" name="search_rmc" value="1">';
...
function rmc_results_before_content() {
	...
	if ((isset($_POST['search_rmc']))){
			...
			foreach ( $mythe_query as $post ) : setup_postdata( $post );
				...
				$list .= '	<hr style="width:100%;'.$height.'border: '.($epaisseur_bordure-1).'px solid '.$couleur_bordure.';clear:both;background-color: '.$couleur_bordure.'"><article class="grid-entry" style="float: left;margin: 0 20px 20px 0;width: 100%;"><a style="float:left;margin-right:10px;" href="' . get_permalink($post->ID) . '"><img src="'.$feat_image[0].'"></a><span><a href="'. get_permalink($post->ID) . '"><b>' . ucfirst($post->post_title) . '</b> - '.get_the_date('d/m/Y',$post->ID).' - <span>'.substr($new_content,0,200).'...</span></a></span></article>';
		...
		if (child){
			...
			child = document.getElementById('main');
